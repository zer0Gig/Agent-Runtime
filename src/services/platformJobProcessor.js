...
export async function runFeedbackLoop(
  ...
  while (Date.now() < deadline) {
    ...
    if (userMessages.length > 0) {
      ...
      console.log(`[PlatformProcessor] Approval validator: approved=${approvalResult.approved}, confidence=${approvalResult.confidence}`);
      ...
      if (approvalResult.approved && approvalResult.confidence >= 0.7) {
        ...
        console.log(`[PlatformProcessor] Approval confirmed via Groq validator — proceeding to release milestone ${milestoneIndex + 1}`);
        await postChat(
          jobId,
